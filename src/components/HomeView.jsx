...
const HomeView = ({ user }) => {
  return (
    <div className="flex flex-col items-center py-10 md:py-16 px-4 w-full bg-[#3E2F49] space-y-16 md:space-y-24">
      {/* Welcome Section */}
      {user && (
        <section className="text-center">
            <h1 className="font-magic text-4xl md:text-6xl text-[#E9D5FF] mb-2 tracking-widest px-2">Welcome, {user.username}!</h1>
            <p className="text-sm md:text-base text-white font-bold opacity-80 uppercase tracking-tighter">Your adventure in Word Valley awaits.</p>
        </section>
      )}

      {/* Teaser Video */}
      <section className="w-full max-w-5xl flex flex-col items-center gap-6 md:gap-10">
        <h2 className="font-magic text-3xl md:text-5xl text-[#E9D5FF] tracking-wide text-center">Teaser Video</h2>
        <div className="relative w-full max-w-4xl aspect-video rounded-2xl md:rounded-3xl overflow-hidden border-4 md:border-8 border-black/20 shadow-2xl bg-black/40 group">
          <div className="absolute inset-0 bg-cover bg-center opacity-60 group-hover:opacity-80 transition-opacity flex items-center justify-center overflow-hidden">
             ...
             <div className="w-full h-full bg-[#1E1428] flex items-center justify-center">
                <p className="font-magic text-3xl md:text-6xl text-white/20 select-none text-center px-4">MAGIC READER ARTWORK</p>
             </div>
          </div>
          <div className="absolute inset-0 flex items-center justify-center">
            <button className="w-16 h-16 md:w-24 md:h-24 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center hover:scale-110 transition-transform border-2 md:border-4 border-white/40 shadow-2xl">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 md:h-12 md:w-12 text-white fill-current" viewBox="0 0 24 24">
                <path d="M8 5v14l11-7z" />
              </svg>
            </button>
          </div>
          {/* Progress bar like in screenshot */}
          <div className="absolute bottom-4 md:bottom-8 left-4 md:left-12 right-4 md:right-12 flex gap-2 md:gap-4 items-center">
             <div className="h-2 md:h-4 bg-gray-400/50 rounded-full flex-grow relative overflow-hidden backdrop-blur-sm">
                <div className="absolute top-0 left-0 h-full w-2/3 bg-gray-200"></div>
             </div>
             <div className="w-12 md:w-24 h-2 md:h-4 bg-gray-400/50 rounded-full backdrop-blur-sm"></div>
          </div>
        </div>
      </section>

      {/* Story section */}
      <section className="w-full max-w-5xl flex flex-col items-center gap-8 md:gap-12 text-center pb-12">
        <h2 className="font-magic text-3xl md:text-5xl text-[#E9D5FF] tracking-wide uppercase">Story</h2>
        <div className="space-y-6 md:space-y-10 text-base md:text-xl font-bold leading-relaxed text-[#E9D5FF]/90 max-w-5xl px-4 md:px-6">
          <p>
        ...
        <button className="mt-8 bg-[#2D1E3A] border border-white/20 hover:bg-[#1E1428] py-4 md:py-6 px-12 md:px-24 text-base md:text-lg uppercase font-black tracking-[0.2em] shadow-2xl hover:scale-105 transition-all text-white active:scale-95">
          PLAY NOW
        </button>
      </section>
    </div>
